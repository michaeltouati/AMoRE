...
module diagnostics
  ...
  elemental function Kioni_cross_section(E_ioniK, U, J)
    ...
    Gr = (1._PR+(2._PR*J))*((U+J)**2._PR)*(((1._PR+U)*(U+(2._PR*J))*((1._PR+J)**2._PR))**1.5_PR) &
      &/ ( (U+(2._PR*J))*((1._PR+J)**2._PR) * &
          &((J**2._PR)*(1._PR+(2._PR*J))+(U*(U+(2._PR*J))*(1.+(J**2_PR))))**1.5_PR)
  ...
  subroutine Kalpha_emission(Kalpha_tab, norm, d_t, Z, ni, eps_a, phi_n,&
    ...
    real(PR), dimension(1:N_x,1:N_z), intent(inout)                                            :: n_K_Holes
    real(PR), dimension(1:N_x,1:N_z), intent(inout)                                            :: n_Kalpha
    real(PR), dimension(1:N_x,1:N_z), intent(inout)                                            :: n_Kbeta
    real(PR), dimension(1:N_x,1:N_z), intent(inout)                                            :: ioniz_rate
    ! locals
    integer                                                                                    :: k,i
  ...
  subroutine plasma_diagnostics(eta_tab)
    ...
    open (unit=1,file ='results/'//trim(simu)//'/zeffvsTe[eV].dat'                           ,&
      form='formatted',status='unknown')
    open (unit=2,file ='results/'//trim(simu)//'/electron_capacity[SI]vsTe[eV].dat'          ,&
      form='formatted',status='unknown')
    open (unit=3,file ='results/'//trim(simu)//'/ion_capacity[SI]vsTi[eV].dat'               ,&
      form='formatted',status='unknown')
    open (unit=4,file ='results/'//trim(simu)//'/resistivity[SI]vsTe[eV]_Te_eq_Ti.dat'       ,&
      form='formatted',status='unknown')
    open (unit=7,file ='results/'//trim(simu)//'/resistivity[SI]vsTe[eV]_Ti_eq_Tamb.dat'     ,&
      form='formatted',status='unknown')
    open (unit=8,file ='results/'//trim(simu)//'/conductivity[SI]vsTe[eV]_Te_eq_Ti.dat'      ,&
      form='formatted',status='unknown')
    open (unit=9,file ='results/'//trim(simu)//'/conductivity[SI]vsTe[eV]_Ti_eq_Tamb.dat'    ,&
      form='formatted',status='unknown')
    open (unit=10,file='results/'//trim(simu)//'/G[SI]vsTe[eV]_Te_eq_Ti.dat'                 ,&
      form='formatted',status='unknown')
    open (unit=11,file='results/'//trim(simu)//'/G[SI]vsTe[eV]_Ti_eq_Tamb.dat'               ,&
      form='formatted',status='unknown')
    open (unit=12,file='results/'//trim(simu)//'/ang_coll_rate[s-1]vsEps[keV].dat'           ,&
      form='formatted',status='unknown')
    open (unit=13,file='results/'//trim(simu)//'/stopping_power[keV_Microns-1]vsEps[keV].dat',&
      form='formatted',status='unknown')
    do i=-2000,4000
  ...
  subroutine electron_diagnostics(eps_tab,xa,epsa)
    ...
    open (unit=1,file ='results/'//trim(simu)//'/electron_beam_kinetic_energy_spectrum.dat'  ,&
      form='formatted',status='unknown')
    open (unit=2,file ='results/'//trim(simu)//'/electron_beam_angular_distribution.dat'     ,&
      form='formatted',status='unknown')
    open (unit=3,file ='results/'//trim(simu)//'/electron_beam_transverse_distribution.dat'  ,&
      form='formatted',status='unknown')
    open (unit=4,file ='results/'//trim(simu)//'/electron_beam_longitudinal_distribution.dat',&
      form='formatted',status='unknown')
    !
    ...
    do i=1,N_x,1
      ...
      do k=1,N_theta,1
        ...
        dN_dtheta(i,k) = alpha * exp( ( alpha_x * sin(theta(k)) ) + ( alpha_z * cos(theta(k)) ) ) &
        / ( 4.*pi * sinh(alpha) )
  ...
  subroutine initialize_diagnostics(Kalphatab)
    ...
    integer                                    :: i
    open (unit=10 ,file='results/'//trim(simu)//'/psi0_z[cm-3_keV-1].dat'     ,&
      form='formatted',status='unknown')
    open (unit=11 ,file='results/'//trim(simu)//'/psi1x_z[cm-3_keV-1].dat'    ,&
      form='formatted',status='unknown')
    open (unit=12,file='results/'//trim(simu)//'/psi1z_z[cm-3_keV-1].dat'     ,&
      form='formatted',status='unknown')
    open (unit=13,file='results/'//trim(simu)//'/psi0_x[cm-3_keV-1].dat'      ,&
      form='formatted',status='unknown')
    open (unit=14,file='results/'//trim(simu)//'/psi1x_x[cm-3_keV-1].dat'     ,&
      form='formatted',status='unknown')
    open (unit=15,file='results/'//trim(simu)//'/psi1z_x[cm-3_keV-1].dat'     ,&
      form='formatted',status='unknown')
    open (unit=20,file='results/'//trim(simu)//'/nb[cm-3].dat'                ,&
      form='formatted',status='unknown')
    open (unit=21,file='results/'//trim(simu)//'/jb_x[A_cm-2].dat'            ,&
      form='formatted',status='unknown')
    open (unit=22,file='results/'//trim(simu)//'/jb_z[A_cm-2].dat'            ,&
      form='formatted',status='unknown')
    open (unit=23,file='results/'//trim(simu)//'/E_x[V_m-1].dat'              ,&
      form='formatted',status='unknown')
    open (unit=24,file='results/'//trim(simu)//'/E_z[V_m-1].dat'              ,&
      form='formatted',status='unknown')
    open (unit=25,file='results/'//trim(simu)//'/B_y[Tesla].dat'              ,&
      form='formatted',status='unknown')
    open (unit=26,file='results/'//trim(simu)//'/We[erg_s-1_cm-3].dat'        ,&
      form='formatted',status='unknown')
    open (unit=27,file='results/'//trim(simu)//'/Te[eV].dat'                  ,&
      form='formatted',status='unknown')
    open (unit=28,file='results/'//trim(simu)//'/Wi[erg_s-1_cm-3].dat'        ,&
      form='formatted',status='unknown')
    open (unit=29,file='results/'//trim(simu)//'/Ti[eV].dat'                  ,&
      form='formatted',status='unknown')
    open (unit=30,file='results/'//trim(simu)//'/resis[Ohm.m].dat'            ,&
      form='formatted',status='unknown')
    open (unit=31,file='results/'//trim(simu)//'/Kappa_e[erg_m-1_K-1_s-1].dat',&
      form='formatted',status='unknown')
    open (unit=32,file='results/'//trim(simu)//'/K_shell_ioniz_rate_[s-1].dat',&
      form='formatted',status='unknown')
    open (unit=33,file='results/'//trim(simu)//'/n_Kalpha[cm-3].dat'          ,&
      form='formatted',status='unknown')
    open (unit=34,file='results/'//trim(simu)//'/n_Kbeta[cm-3].dat'           ,&
      form='formatted',status='unknown')
    open (unit=35,file='results/'//trim(simu)//'/ni[cm-3].dat'                ,&
      form='formatted',status='unknown')
    open (unit=40,file='results/'//trim(simu)//'/U_e[J].dat'                  ,&
      form='formatted',status='unknown')
    open (unit=41,file='results/'//trim(simu)//'/U_b[J].dat'                  ,&
      form='formatted',status='unknown')
    open (unit=42,file='results/'//trim(simu)//'/Ud_col[J].dat'               ,&
      form='formatted',status='unknown')
    open (unit=43,file='results/'//trim(simu)//'/Ud_res[J].dat'               ,&
      form='formatted',status='unknown')
    open (unit=44,file='results/'//trim(simu)//'/U_el[J].dat'                 ,&
      form='formatted',status='unknown')
    open (unit=45,file='results/'//trim(simu)//'/U_ma[J].dat'                 ,&
      form='formatted',status='unknown')
    open (unit=46,file='results/'//trim(simu)//'/U_sf[J].dat'                 ,&
      form='formatted',status='unknown')
    open (unit=47,file='results/'//trim(simu)//'/U_sb[J].dat'                 ,&
      form='formatted',status='unknown')
    open (unit=48,file='results/'//trim(simu)//'/U_su[J].dat'                 ,&
      form='formatted',status='unknown')
    open (unit=49,file='results/'//trim(simu)//'/U_sd[J].dat'                 ,&
      form='formatted',status='unknown')
    open (unit=50,file='results/'//trim(simu)//'/ne[cm-3].dat'                ,&
      form='formatted',status='unknown')
    open (unit=52,file='results/'//trim(simu)//'/je_x[A_cm-2].dat'            ,&
      form='formatted',status='unknown')
    open (unit=53,file='results/'//trim(simu)//'/je_z[A_cm-2].dat'            ,&
      form='formatted',status='unknown')
    ! Open, read and store the file 'sources/data/Kalpha_tab.dat' in the table Kalphatab :
    open (unit=51,file='sources/data/Kalpha_tab.dat'          ,&
      form='formatted',status='unknown')      
    read(51,*)
    do i=1,79,1
  ...
  subroutine energy_balance(diag_condition, N_t, time, d_t, norm, eps_a, phi_n,&
    ...
    real(PR), dimension(1:N_eps), intent(in)                                 :: eps_a
    real(PR), dimension(forward:backward,psi0:psi1z,-1:N_x+2,-1:N_z+2,-1:N_eps+2), intent(in) :: phi_n
    real(PR), dimension(1:N_x,1:N_z), intent(in)                             :: p_depos
    real(PR), dimension(1:N_x,1:N_z), intent(in)                             :: p_lost_col
    real(PR), dimension(1:N_x,1:N_z), intent(in)                             :: p_lost_res
    real(PR), dimension(1:N_x,1:N_z), intent(in)                             :: E_x, E_z, B_y_n
    real(PR), intent(inout)                                                  :: U_e, U_b, U_d
    real(PR), intent(inout)                                                  :: U_d_col, U_d_res
    real(PR), intent(inout)                                                  :: U_el, U_ma
  ...
  subroutine write_results(eta_tab, A, Z, ni, n_b, j_b_x_n, j_b_z_n,&
    ...
    real(PR), dimension(0:N_x+1,0:N_z+1), intent(in)                                          :: Te, Ti
    real(PR), dimension(1:N_x,1:N_z), intent(in)                                              :: n_b
    real(PR), dimension(1:N_x,1:N_z), intent(in)                                              :: j_b_x_n
    real(PR), dimension(1:N_x,1:N_z), intent(in)                                              :: j_b_z_n
    real(PR), dimension(1:N_x,1:N_z), intent(in)                                              :: j_r_x_n
    real(PR), dimension(1:N_x,1:N_z), intent(in)                                              :: j_r_z_n
    real(PR), dimension(1:N_x,1:N_z), intent(in)                                              :: E_x, E_z
    real(PR), dimension(1:N_x,1:N_z), intent(in)                                              :: B_y_n
    real(PR), dimension(1:N_x,1:N_z), intent(in)                                              :: n_Kalpha
    real(PR), dimension(1:N_x,1:N_z), intent(in)                                              :: n_Kbeta
    real(PR), dimension(1:N_x,1:N_z), intent(in)                                              :: p_depos
    real(PR), dimension(1:N_x,1:N_z), intent(in)                                              :: ioniz_rate
    real(PR), dimension(forward:backward,psi0:psi1z,-1:N_x+2,-1:N_z+2,-1:N_eps+2), intent(in) :: phi_n
