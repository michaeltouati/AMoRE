...
module vfp
  ...
  subroutine spatial_derivatives(eps_a,phi_n,dF_dz,dF_dx)
    ...
    ! locals
    integer                                                                                   :: m,l,k,i
    real(PR), dimension(psi0:psi1z)                                                           :: phi_r2,phi_r
    real(PR), dimension(psi0:psi1z)                                                           :: phi_m
    real(PR), dimension(psi0:psi1z)                                                           :: phi_l,phi_l2
  ...
  subroutine energy_derivative(E_x,E_z,eps_a, Sv_a,Sv_a_lp1,phi_n,dF_deps)
    ...
    real(PR), dimension(psi0:psi1z)                                          :: phi_l2,phi_l,phi_m
    real(PR), dimension(psi0:psi1z)                                          :: phi_r,phi_r2
    ...
    if (coll_implicit_scheme.eqv..false.) then
      !$omp PARALLEL DO DEFAULT(NONE) &
      !$omp& SHARED(N_z, N_x, N_eps, backward) &
  ...
  subroutine source_terms(E_x,E_z,B_y,eps_a,nu_a,phi_n,gama_E,gama_B,gama_nu)
    ...
    real(PR), dimension(forward:backward,psi0:psi1z,-1:N_x+2,-1:N_z+2,-1:N_eps+2), intent(in) :: phi_n
    real(PR), dimension(forward:backward,psi0:psi1z,1:N_x,1:N_z,1:N_eps), intent(out)         :: gama_E
    real(PR), dimension(forward:backward,psi0:psi1z,1:N_x,1:N_z,1:N_eps), intent(out)         :: gama_B 
    ...
    integer                         :: m,l,k,i
    !$omp  PARALLEL DO DEFAULT(NONE) &
    !$omp& SHARED(N_eps, N_z, N_x, backward) &
    !$omp& SHARED(phi_n, B_y, E_z, E_x, eps_a, nu_a) &
  ...
  subroutine update(phi_n, d_t, dF_dx, dF_dz, dF_deps, gama_e, gama_b, gama_nu, Sv_a, Sv_a_lp1, nu_a, phi_np1)
    ...
    real(PR), dimension(forward:backward,psi0:psi1z,1:N_x,1:N_z,1:N_eps), intent(in)           :: dF_dx
    real(PR), dimension(forward:backward,psi0:psi1z,1:N_x,1:N_z,1:N_eps), intent(in)           :: dF_dz
    real(PR), dimension(forward:backward,psi0:psi1z,1:N_x,1:N_z,1:N_eps), intent(in)           :: dF_deps
    real(PR), dimension(forward:backward,psi0:psi1z,1:N_x,1:N_z,1:N_eps), intent(in)           :: gama_e
    real(PR), dimension(forward:backward,psi0:psi1z,1:N_x,1:N_z,1:N_eps), intent(in)           :: gama_b
    real(PR), dimension(forward:backward,psi0:psi1z,1:N_x,1:N_z,1:N_eps), intent(in)           :: gama_nu
    ...
    !$omp PARALLEL DO DEFAULT(NONE) &
    !$omp SHARED(backward, N_eps, N_z, N_x, d_t, phi_n, phi_np1) &
    ...
    if (coll_implicit_scheme) then
      ...
      !$omp PARALLEL DO DEFAULT(NONE) &
      !$omp SHARED(backward, N_eps, N_z, N_x, d_t, d_eps, phi_n, phi_np1) &
  ...
  subroutine boundary_cond(norma, time, dt, epsa, Ue, Usf, Usb, Usu, Usd, phin)
    ...
    real(PR), dimension(1:N_eps), intent(in)                                                     :: epsa
    real(PR), intent(inout)                                                                      :: Ue, Usf
    real(PR), intent(inout)                                                                      :: Usb
    real(PR), intent(inout)                                                                      :: Usu, Usd
    real(PR), dimension(forward:backward,psi0:psi1z,-1:N_x+2,-1:N_z+2,-1:N_eps+2), intent(inout) :: phin
    ! locals
    real(PR), dimension(psi0:psi1z)                                                              :: phitemp
    real(PR), dimension(psi0:psi1z)                                                              :: Fx_l, Fx_r
    real(PR), dimension(psi0:psi1z)                                                              :: Fz_l, Fz_r
    integer                                                                                      :: m, i, k, l
    integer                                                                                      :: psi_mu
    real(PR),dimension(:,:,:),allocatable                                                        :: usu_temp_x
    real(PR),dimension(:,:,:),allocatable                                                        :: usd_temp_x
    real(PR),dimension(:,:),allocatable                                                          :: ue_temp_z
    real(PR),dimension(:,:),allocatable                                                          :: usf_temp_z
    real(PR),dimension(:,:),allocatable                                                          :: usb_temp_z
    !
    ...
    !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    !$omp PARALLEL DO DEFAULT(NONE) &
    !$omp SHARED(backward, N_z, N_x, N_eps, phin) &
    ...
    !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    !$omp PARALLEL DO DEFAULT(NONE) &
    !$omp SHARED(backward, N_z, N_x, N_eps, d_eps, dt, d_z, sim_box_thickness, norma) &
    ...
    do l=1,N_eps,1
      do i=-1,N_x+2,1
        ...
        ! for fast electrons propagating forward :                 
        ...
        if (backward.eq.2) then
          ! if rear side refluxing : initialization of refluxing electrons propagating backward 
          ! assuming a specular reflection of fast electrons propagating forward
          if (phin(forward,psi1z,i,N_z,l) > 0._PR) then
          ...
          if (phin(backward,psi1z,i,N_z,l) > 0._PR) then
            phin(backward,psi0:psi1z,i,N_z+1,l) = phin(backward,psi0:psi1z,i,N_z+1,l)&
            + phin(backward,psi0:psi1z,i,N_z,l)
          end if
          phin(backward,psi0:psi1z,i,N_z+2,l) = phin(backward,psi0:psi1z,i,N_z+1,l)
        end if
        ! Energy diagnostics :
        if ((i.ne.0).and.(i.ne.(-1)).and.(i.ne.(N_x+2)).and.(i.ne.(N_x+1))) then
          ...
          ! irradiated side refluxing : initialization of refluxing electrons propagating forward 
          ! assuming a specular reflection of fast electrons propagating forward
