...
real(PR)                                            :: div_q
!$omp  PARALLEL DO DEFAULT(SHARED) PRIVATE(i, k, kapa_iph_k, kapa_imh_k,&
!$omp& kapa_i_kph, kapa_i_kmh,div_q) COLLAPSE(2)
do k=1,N_z,1
do i=1,N_x,1         
...
real(PR), dimension(1:N_x,1:N_z), intent(in)                                              :: jbx, jbz, jrx, jrz
real(PR), dimension(1:N_x,1:N_z), intent(out)                                             :: n_b, Pdepos
real(PR), dimension(1:N_x,1:N_z), intent(out)                                             :: Plost_col, Plost_res
! locals
integer                                                                                   :: k, i
